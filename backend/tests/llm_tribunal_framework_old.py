...
class RubricResult:
    ...
    warnings: List[str]
    score: float  # 0-1
    consistency_issues: List[str] = None  # Logic inconsistencies caught programmatically
...
class ConsistencyChecker:
    ...
    Fast, free, deterministic logic validation.
    Catches obvious contradictions before expensive LLM calls.
    """
    
    @staticmethod
    def check_goal_proximity_consistency(response: Dict[str, Any]) -> List[str]:
        """
        Check for logical inconsistencies in goal proximity response.
        ...
        score = response.get("proximity_score")
        reasoning = response.get("reasoning", "").lower()
        evidence = response.get("evidence", "").lower()
        
        if score is None:
            return issues  # Rubric will catch this
        
        # High score but negative reasoning
        negative_phrases = ["no progress", "not started", "nothing done", "no work", 
                           "hasn't begun", "not implemented", "not created", "nothing implemented",
                           "no files"]
        if score > 0.8 and any(phrase in reasoning for phrase in negative_phrases):
            issues.append(f"High score ({score}) contradicts negative reasoning: '{reasoning[:50]}'")
        
        if score > 0.8 and any(phrase in evidence for phrase in negative_phrases):
            issues.append(f"High score ({score}) contradicts negative evidence: '{evidence[:50]}'")
        
        # Low score but positive reasoning
        positive_phrases = ["complete", "finished", "done", "implemented", "working", 
                           "successful", "passing", "deployed"]
        if score < 0.3 and any(phrase in reasoning for phrase in positive_phrases):
            issues.append(f"Low score ({score}) contradicts positive reasoning: '{reasoning[:50]}'")
        
        if score < 0.3 and any(phrase in evidence for phrase in positive_phrases):
            issues.append(f"Low score ({score}) contradicts positive evidence: '{evidence[:50]}'")
        
        # Mid-range score with extreme language
        extreme_complete = ["100%", "all done", "completely finished", "fully implemented"]
        extreme_none = ["0%", "nothing", "none", "no files"]
        
        if 0.3 <= score <= 0.7:
            if any(phrase in reasoning or phrase in evidence for phrase in extreme_complete):
                issues.append(f"Mid score ({score}) but uses extreme completion language")
            if any(phrase in reasoning or phrase in evidence for phrase in extreme_none):
                issues.append(f"Mid score ({score}) but uses extreme zero-progress language")
        
        # Confidence vs evidence mismatch
        confidence = response.get("confidence")
        if confidence and confidence > 0.9 and ("uncertain" in reasoning or "maybe" in reasoning or "possibly" in reasoning):
            issues.append(f"High confidence ({confidence}) but uncertain language in reasoning")
        
        if confidence and confidence < 0.5 and ("definitely" in reasoning or "certainly" in reasoning or "clearly" in reasoning):
            issues.append(f"Low confidence ({confidence}) but definitive language in reasoning")
...
class RubricValidator:
    ...
    def validate_goal_proximity_response(response: Dict[str, Any]) -> RubricResult:
        ...
        # Stage 0: Run consistency checks FIRST (before structural validation)
        consistency_issues = ConsistencyChecker.check_goal_proximity_consistency(response)
        if consistency_issues:
            # Consistency issues are treated as errors
            errors.extend(consistency_issues)
            score -= 0.3 * len(consistency_issues)  # Heavy penalty for logic errors
    ...
    def validate_prompt_improvement_response(response: Dict[str, Any]) -> RubricResult:
        ...
        return RubricResult(
            ...
            warnings=warnings,
            score=max(0.0, score),
            consistency_issues=[]  # No consistency check for prompt improvement yet
        )
